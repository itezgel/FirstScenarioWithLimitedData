*&---------------------------------------------------------------------*
*& Report ZCHECK_AND_FIX_DAILY_FLC_PA
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZCHECK_AND_FIX_DAILY_PA.
...
START-OF-SELECTION.
  ...
  DATA : lt_log TYPE TABLE OF zpa_logging,
         ls_log TYPE zpa_logging.
...
**** check PO errors*************
  SELECT * FROM zpa_logging INTO TABLE lt_log WHERE result_flag = 'E' AND transaction_type LIKE '%PO%' AND
...
*                 WITH SELECTION-TABLE lt_seltab
      ...
      SUBMIT  zrun_gr_in_for_po_ekpo_pa  "VIA SELECTION-SCREEN
*                 WITH SELECTION-TABLE lt_seltab
                  WITH s_po IN range_tab
                 EXPORTING LIST TO MEMORY
                 AND RETURN.
...
************* Prod Errors************
  ...
  SELECT * FROM zpa_logging INTO TABLE lt_log WHERE result_flag = 'E' AND transaction_type LIKE 'PROD' AND
                                            ( date_posted BETWEEN lv_date1 AND lv_date2 )  .
  ...
  IF sy-subrc IS INITIAL.
    ...
    IF lt_prod[] IS NOT INITIAL.
      SUBMIT  zcomplete_production_cycle_pa  "VIA SELECTION-SCREEN
*                 WITH SELECTION-TABLE lt_seltab
...
************* SO Errors************
  ...
  SELECT * FROM zpa_logging INTO TABLE lt_log WHERE result_flag = 'E' AND transaction_type LIKE 'SO' AND
                                            ( date_posted BETWEEN lv_date1 AND lv_date2 )  .
  IF sy-subrc IS INITIAL.
    DESCRIBE TABLE lt_log LINES lv_count.
    WRITE : lv_count,'Error records in SO Cycle'.
    ...
    IF lt_so[] IS NOT INITIAL.
      SUBMIT  zrun_delivery_billing_for_sopa  "VIA SELECTION-SCREEN
*                 WITH SELECTION-TABLE lt_seltab
