FUNCTION zdelivery_pgi_datagen_pa.
*"--------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
******************************  check if Delivery block exists from outside the prgm
  DATA : lv_lifsk   TYPE lifsk,
         block_msg  TYPE char255,
         block_flag TYPE char1.
  SELECT SINGLE lifsk INTO lv_lifsk FROM vbak WHERE vbeln = so_num.
  IF  lv_lifsk IS NOT INITIAL. " Meaning block was not needed but blocked due to config
    CALL FUNCTION 'ZREMOVE_DELIVERYBLOCK' DESTINATION 'NONE'
      EXPORTING
        sonum = so_num
      IMPORTING
        flag  = block_flag
        msg   = block_msg.
    IF block_flag = 'E'.
      CONCATENATE msg block_msg INTO msg SEPARATED BY space.
*              CONTINUE.
    ENDIF.
  ENDIF.
***          End of delivery block check


***check and release customer credit blck for the SO by i065658 as on 07/12/2017**
  DATA : release_subrc TYPE subrc.
  IF so_num IS NOT INITIAL.
    CALL FUNCTION 'ZRELEASE_CREDIT_BLOCK' DESTINATION 'NONE'
      EXPORTING
        vbeln = so_num
      IMPORTING
        subrc = release_subrc.
    IF release_subrc IS NOT INITIAL.
      flag = 2.
      CONCATENATE so_num 'has Customer Credit Block' INTO msg SEPARATED BY space.
      RETURN.
    ENDIF.
  ENDIF.

**      end of credit release

***changes by i065658 as of 16/11/2017***
  cl_fqmc_active=>init_config_buffer( ).
  cl_mcs_adaptor_fqm=>get_adaptor( )->get_hierarchy(
    EXPORTING
      iv_proc_step =  'VA'   " Event for which updating should take place
      iv_modus     =  ' '   " Mode of info structure updating in LIS
*  RECEIVING
*    rv_hierarchy =     " Hierarchy (document header, item,...) of statistics update
  ).
*GET_HIERARCHY
****************end of changes as 16/11/2017****
