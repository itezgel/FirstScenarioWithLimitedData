*&---------------------------------------------------------------------*
*& Report ZCOMPLETE_PRODUCTION_CYCLE_PA
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zcomplete_production_cycle_pa.
...
***to update log
    SELECT SINGLE * FROM zpa_logging INTO ls_datagen_log WHERE documentno =  order_number AND transaction_type = 'PROD'." AND result_flag = 'E'.

    wa_orders-order_number = ls_aufk-aufnr. "order_number.
    APPEND wa_orders TO it_orders.
    CLEAR wa_orders.
    CALL FUNCTION 'ZRELEASE_PRODORDER_PA'
      IMPORTING
        flag              = rel_flag
        msg               = rel_msg
        es_return         = es_return
    ...
    ELSEIF rel_flag = 'S'. "Confirm activities if PO released.
      ...
      IF sy-subrc IS NOT INITIAL.
        ...
        CONTINUE.
      ENDIF.
      CALL FUNCTION 'ZCONFIRM_PRODORDER_PA' "'ZCONFIRM_PRODORDER_NEW'
        EXPORTING
