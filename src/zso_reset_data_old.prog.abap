*&---------------------------------------------------------------------*
*& Report ZSO_RESET_DATA_1
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ZSO_RESET_DATA_OLD.
...
*      wait = 'X'.
** exit.
  CONCATENATE 'Billing document cancelled for the sales order:' p_vbeln
...
ELSE.
  CONCATENATE 'Error in canceling billing doc - ' lv_bill_doc ' of Sales order - ' p_vbeln INTO ls_message.
  APPEND ls_message TO lt_message.
  CLEAR: ls_message.
...
*
*CLEAR: lv_argument.
*CONCATENATE sy-mandt p_vbeln INTO lv_argument.
*
** Sales organization
...
*      OTHERS      = 0.
*
*  CONCATENATE 'Billing & Delivery Block is set to Sales Order:' p_vbeln
*  INTO ls_message SEPARATED BY ''.
*  APPEND ls_message TO lt_message.
*  CLEAR: ls_message.
*
...
IF lt_message IS NOT INITIAL.
  ...
  ENDLOOP.
ELSE.
  WRITE : / 'Data reset for SO', p_vbeln  .
ENDIF.
