...
*&
*&---------------------------------------------------------------------*
REPORT ZRUN_ONLYPGI_BILLING_FOR_SO.
...
*    *      CALL FUNCTION 'ZCREATEDELIVERYDOC_DATAGEN'
      CALL FUNCTION 'ZPERFORM_PGI_OF_DELIVERYDOC'
        EXPORTING
          so_num         = lv_sonum
          actual_gidate  = lv_postingdate "ls_log-date_posted
        IMPORTING
          flag           = flag
          msg            = deliverydoc_msg
      ...
      IF flag = 2 AND deliveryno IS INITIAL.
        NEW-LINE.
        WRITE : lv_sonum , 'Delivery Doc  creation failed'.
        WRITE : lv_sonum, deliverydoc_msg.
        NEW-LINE.
...
*        WRITE : 'Delivery Doc', deliveryno, 'Created successfully'.
        ...
        IF sy-subrc IS INITIAL OR billingdocument IS INITIAL.
          ...
          APPEND ls_dbupdate_so TO lt_dbupdate_so.
          WRITE : lv_sonum,'Delivery doc:', deliveryno, 'created.'.
          NEW-LINE.
           Write :lv_sonum, 'Billing Doc:', billingdoc_msg.
...
**DELETE lt_sales_order where material NS 'MZ-FG-C'.  " delete this.
