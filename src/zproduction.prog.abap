...
*DELETE lt_sales_order where material NE 'MZ-FG-C900'.  " delete this.
  ...
  LOOP AT lt_sales_order INTO ls_sales_order .
    ...
    ls_mat-plant = ls_sales_order-sales_org.  "Added by C5252040
    SELECT SINGLE mtart
      FROM mara
      INTO ls_mat-mtart
     WHERE matnr = ls_mat-material.
...
*  LS_MAT-MATERIAL,
*  LS_MAT-QTY.
        LOOP AT lt_mat2 INTO ls_mat2
          WHERE material = ls_mat-material
            AND record_date = temp_date
        ...
        DELETE lt_mat
         WHERE material = ls_mat-material
           AND record_date = temp_date
           AND plant = ls_mat-plant. " and SOHDR_SEQNO = ls_mat-SOHDR_SEQNO.
        CLEAR ls_mat.
      ENDLOOP.
...
***   create prod order.
      ...
      is_prodorder-plant = ls_mat3-plant.
...
*      CALL FUNCTION 'ZRELEASE_PRODORDER_NEW'
*        IMPORTING
*          flag              = rel_flag
*          msg               = rel_msg
*          es_return         = es_return
