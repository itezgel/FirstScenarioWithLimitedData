*&---------------------------------------------------------------------*
*& Report ZPROCURETOPAY
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT zprocuretopay_backdated.
...
*DELETE lt_sales_order where material NE 'MZ-FG-C950'.  " delete this.
*    DELETE lt_sales_order where material NS 'MZ-FG-C' and material NS 'MZ-TG' .  " delete this.
  DELETE lt_sales_order WHERE status EQ 'S' OR status EQ 'E' OR status EQ 'P'. "P for predictive Accounting
*DELETE lt_sales_order WHERE status EQ 'P'. "P for predictive Accounting
...
*{   INSERT         E3TK900539                                        1
    ls_mat-plant = ls_sales_order-sales_org.
*}   INSERT
...
*\    LOOP AT lt_mat2 INTO ls_mat2 WHERE material = ls_mat-material AND record_date = ls_mat-record_date.
      LOOP AT lt_mat2 INTO ls_mat2 WHERE material = ls_mat-material AND record_date = ls_mat-record_date AND Plant = ls_mat-plant.
*}   REPLACE
...
*\    DELETE lt_mat WHERE material = ls_mat-material AND record_date = ls_mat-record_date.
      DELETE lt_mat WHERE material = ls_mat-material AND record_date = ls_mat-record_date AND plant = ls_mat-plant.
*}   REPLACE
      CLEAR ls_mat.
    ENDLOOP.
  ENDIF.
  ...
  IF lt_mat3[] IS NOT INITIAL.
    LOOP AT lt_mat3 INTO ls_mat3.
      IF ls_mat3-mtart = 'HAWA'.
        ...
        SELECT  * FROM zdatagen_procure INTO TABLE lt_procure  WHERE mtart EQ ls_mat3-mtart AND matnr EQ ls_mat3-material AND plant EQ ls_mat3-plant.
...
*          READ TABLE lt_mat3 INTO ls_mat3 WITH KEY material = ls_procure-component."MATERIAL.
          ...
          CALL FUNCTION 'ZCREATEPO_DATAGEN1'
            IMPORTING
              msg        = po_msg
            TABLES
              it_pohdr   = it_pohdr
...
*      CLEAR : ls_p2plog.
      ...
      ELSEIF ls_mat3-mtart = 'FERT'.  " Create PO fro components of these FERT type materials
        ...
        SELECT * FROM zdatagen_procure INTO  TABLE lt_procure WHERE matnr = ls_mat3-material AND  mtart EQ ls_mat3-mtart AND plant EQ ls_mat3-plant.
        IF lt_procure[] IS NOT INITIAL.
          LOOP AT lt_procure INTO ls_procure.
