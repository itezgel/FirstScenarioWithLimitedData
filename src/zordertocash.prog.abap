  ...
  IF lt_mat[] IS NOT INITIAL.
    ...
    LOOP AT lt_mat INTO ls_mat.
      LOOP AT lt_mat2 INTO ls_mat2 WHERE material = ls_mat-material
       AND record_date = ls_mat-record_date AND plant = ls_mat-plant.
      ...
      DELETE lt_mat WHERE material = ls_mat-material
         AND record_date = ls_mat-record_date
         AND plant = ls_mat-plant.
      CLEAR ls_mat.
    ENDLOOP.
  ENDIF.
    ...
    LOOP AT lt_material_stock INTO ls_material_stock.
      ...
      CALL FUNCTION 'ZCHECK_MATERIAL_STOCK'
        EXPORTING
          ...
          plant             = ls_material_stock-plant "'1710'
        IMPORTING
          flag              = stck_flag
          msg               = stck_msg
...
**************** Execute each distinct Material records Once*************
  DATA : lt_dbstruc          TYPE TABLE OF zdemo_data, "zdatagen_dbstructure
         wa_dbstuc           TYPE  zdemo_data,
         temp_lt_sales_order TYPE TABLE OF  zdemo_data,
         temp_lt_dbupdate_so TYPE TABLE OF zdemo_data,
         temp_ls_dbupdate_so TYPE zdemo_data,
         lv_flag             TYPE char1,
         wa_mat3             LIKE LINE OF lt_mat3,
         wa_salesorder       LIKE LINE OF lt_sales_order.
  DATA : gt_dbupdate_so TYPE TABLE OF zdemo_data .
...
*          WRITE: ' Billing Document ', billingdocument, ' created successfully'.
        ...
        ENDIF.
        MODIFY ot_dblog FROM ls_dblog TRANSPORTING msg external_document WHERE documentno = lv_sonum.
        MODIFY zdatagen_logging FROM ls_dblog.
        MODIFY zdemo_data FROM ls_dbupdate_so.
        CLEAR : ls_dblog,ls_dbupdate_so.
        NEW-LINE.
        NEW-LINE.
        NEW-LINE.
...
*    MODIFY zdatagen_logging FROM TABLE ot_dblog.
      LOOP AT ot_dblog INTO ls_dblog.
        INSERT zdatagen_logging FROM ls_dblog.
      ENDLOOP.
    ENDIF.
